...
public class DBOracleConnection extends DBConnection {
	

	public DBOracleConnection(Connection p_conn,boolean p_autocommit) throws DBException
	{
		_Conn = p_conn;
	...
	private void getKeyByRowId(DBMetaDataSet p_tableDef, String p_rowId, DBRow p_data)	throws SQLException, DBException
	{
		Statement stmt = null;
		ResultSet rs = null;
		...
		for (String key : p_tableDef.getKeyNameList())
			query += query.length() == 0 ? key : "," + key;

		query = "select " + query + " from " +  p_tableDef.getTableName() + " where rowid='" + p_rowId + "'";
		try
		{
		...
		{
			if(rs != null) DBTools.closeResultSet(rs);
			...
			if (rs.next())
			{
				...
				for (int i = 1; i <= colCount; i++)
				{
					...
					if (colName.equals("ROWID"))
					{
						...
						getKeyByRowId(p_tableDef, rowid, p_data);
						return;
					}
					else
					{
	...
	{
		String cmd = "delete " + p_tableDef.getTableName();
		String cmdWhere = "";
